...
if ( ! function_exists( 'denali_setup' ) ) {
	function denali_setup() {
		/* Load textdomain */
		...
		register_nav_menus( array(
			'primary_menu'   => esc_html__( 'Primary Menu','denali' ),
		...
		add_theme_support( 'custom-background', apply_filters( 'denali_custom_background_args', array(
			'default-color' => 'f5f5f5',
		) ) );

		/* Add support for featured content. */
		add_theme_support( 'featured-content', array(
			'featured_content_filter' => 'denali_get_featured_posts',
			'max_posts' => 6,
		) );
...
add_action( 'after_setup_theme', 'denali_setup' );

/* Logo */
if (!function_exists('denali_logo')) {
	function denali_logo($url = '', $height = 30) {
		if(!$url){
			$url = get_template_directory_uri().'/assets/images/site-logo.png';
		}
		echo '<a href="'.home_url('/').'"><img class="logo" style="height: '.esc_attr($height).'px; width: auto;" src="'.esc_url($url).'" alt="'.esc_attr__('Logo', 'denali').'"/></a>';
	}
}

/* Nav Menu */
if (!function_exists('denali_nav_menu')) {
	function denali_nav_menu($menu_slug = '', $theme_location = '', $container_class = '') {
...
/* Page title */
if (!function_exists('denali_page_title')) {
    function denali_page_title() {
		ob_start();
		if(is_front_page()){
			esc_html_e('Home', 'denali');
		}elseif(is_home()){
			esc_html_e('Blog', 'denali');
		}elseif(is_search()){
			esc_html_e('Search', 'denali');
		}elseif(is_404()){
			esc_html_e('404', 'denali');
		}elseif(is_page()){
			echo get_the_title();
		}elseif (is_archive()) {
			if (is_category()){
				...
				if(is_post_type_archive('therapist')) {
					esc_html_e('SPA Therapist', 'denali');
				} elseif(is_post_type_archive('service')) {
					esc_html_e('SPA Services', 'denali');
				} else {
					single_term_title();
				}
				...
				printf(__('Author: %s', 'denali'), '<span class="vcard">' . get_the_author() . '</span>');
			}elseif (is_day()){
				printf(__('Day: %s', 'denali'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			}elseif (is_month()){
				printf(__('Month: %s', 'denali'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			}elseif (is_year()){
				printf(__('Year: %s', 'denali'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			}elseif (is_tax('post_format', 'post-format-aside')){
				esc_html_e('Aside', 'denali');
			}elseif (is_tax('post_format', 'post-format-gallery')){
				esc_html_e('Gallery', 'denali');
			}elseif (is_tax('post_format', 'post-format-image')){
				esc_html_e('Image', 'denali');
			}elseif (is_tax('post_format', 'post-format-video')){
				esc_html_e('Video', 'denali');
			}elseif (is_tax('post_format', 'post-format-quote')){
				esc_html_e('Quote', 'denali');
			}elseif (is_tax('post_format', 'post-format-link')){
				esc_html_e('Link', 'denali');
			}elseif (is_tax('post_format', 'post-format-status')){
				esc_html_e('Status', 'denali');
			}elseif (is_tax('post_format', 'post-format-audio')){
				esc_html_e('Audio', 'denali');
			}elseif (is_tax('post_format', 'post-format-chat')){
				esc_html_e('Chat', 'denali');
			}else{
				esc_html_e('Archive', 'denali');
			}
		}else {
			echo get_the_title();
		}
...
/* Page breadcrumb */
if (!function_exists('denali_page_breadcrumb')) {
  function denali_page_breadcrumb($home_text = 'Home', $delimiter = '-') {
		global $post;

		if(is_front_page()){
			echo '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Front Page', 'denali');
		}elseif(is_home()){
			echo  '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Blog', 'denali');
		}else{
		...
		if(is_category()){
			...
			echo '<span class="current">' . single_cat_title(esc_html__('Archive by category: ', 'denali'), false) . '</span>';
		}elseif (is_tag()) {
			echo '<span class="current">' . single_tag_title(esc_html__('Posts tagged: ', 'denali'), false) . '</span>';
		}elseif(is_post_type_archive()){
			echo '<span class="current">' . post_type_archive_title( esc_html__('Archive: ', 'denali'), false ) . '</span>';
		}elseif(is_tax()){
			echo '<span class="current">' . single_term_title(esc_html__('Archive by taxonomy: ', 'denali'), false) . '</span>';
		}elseif(is_search()){
			echo '<span class="current">' . esc_html__('Search results for: ', 'denali') . get_search_query() . '</span>';
		}elseif(is_day()){
        ...
        if(get_post_type() == 'product'){
			...
			echo '<span class="current">' . esc_html__('Articles posted by ', 'denali') . $userdata->display_name . '</span>';
		}elseif ( is_404() ) {
			echo '<span class="current">' . esc_html__('Error 404', 'denali') . '</span>';
		}

		if ( get_query_var('paged') ) {
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
				echo ' <span class="bt-deli">' . $delimiter . '</span> ' . esc_html__('Page', 'denali') . ' ' . get_query_var('paged');
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
  }
}
...
if ( ! function_exists( 'denali_post_nav' ) ) {
	function denali_post_nav() {
		...
		<nav class="bt-post-nav clearfix">
			<?php
				previous_post_link('<div class="bt-post-nav--item bt-prev"><span>' . esc_html__('Previous Post', 'denali') . '</span><h3>%link</h3></div>');
				next_post_link('<div class="bt-post-nav--item bt-next"><span>' . esc_html__('Next Post', 'denali') . '</span><h3>%link</h3></div>');
			?>
		</nav>
		<?php
	}
...
if ( ! function_exists( 'denali_paginate_links' ) ) {
	function denali_paginate_links($wp_query) {
...
if ( ! function_exists( 'denali_paging_nav' ) ) {
	function denali_paging_nav() {
